...
if __name__ == "__main__":
    spark = SparkSession.builder \
        .appName("Median Price per m2 per Month by City (HN, HCM)") \
        .getOrCreate()
    ...
    output_base_path = "hdfs://hadoop-namenode:8020/user/batch_views_spark/median_price_per_m2_per_month_by_city"
    local_output_path = "/app/spark_jobs/output_median_price_m2_by_month"
    ...
    try:
        ...
        # --- Tính giá TRUNG VỊ (Median) trên m2
        result = df.groupBy("city_norm", "month") \
                   .agg(
                       # Tính trung vị (phân vị 0.5)
                       percentile_approx("gia_tren_m2", 0.5).alias("gia_trung_vi_m2_trieu"),
                       # Có thể giữ lại trung bình để so sánh
                       avg("gia_tren_m2").alias("gia_tb_m2_trieu")
                   ) \
                   .orderBy("city_norm", "month")

        print("Tính toán hoàn tất. Bắt đầu ghi file...")
        print("Kết quả tính toán (Giá Trung vị / m2 và Giá Trung bình / m2):")
