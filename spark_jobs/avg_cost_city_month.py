# File: spark_jobs/avg_price_per_m2_per_month_by_city_to_es.py
...
ES_INDEX_AVG_PRICE_MONTH_HANOI = "avg_price_m2_month_hanoi"
ES_INDEX_AVG_PRICE_MONTH_HOCHIMINH = "avg_price_m2_month_hochiminh"
...
if __name__ == "__main__":
    spark_app_name = "Avg Price per m2 per Month by City to ES (HN, HCM)"
    ...
    output_hdfs_base_path = "hdfs://hadoop-namenode:8020/user/batch_views_spark/avg_price_per_m2_per_month_by_city_hdfs"
    # Đường dẫn output local (giữ lại nếu bạn vẫn muốn ghi ra local)
    local_output_path = "/app/spark_jobs/output_avg_price_m2_by_month_local"

    print(f"Đọc dữ liệu từ: {input_path}")
    print(f"Ghi kết quả HDFS phân vùng tới: {output_hdfs_base_path}")
    print(
        f"Ghi kết quả Elasticsearch tới các index: '{ES_INDEX_AVG_PRICE_MONTH_HANOI}' và '{ES_INDEX_AVG_PRICE_MONTH_HOCHIMINH}'")
    ...
    try:
        ...
        if processed_df.rdd.isEmpty():
            ...
            # Tính giá trung bình trên m2 theo tháng và thành phố
            result_df = df_with_price_m2_month.groupBy("city_norm", "month") \
                .agg(avg("gia_tren_m2").alias("gia_tb_m2_trieu")) \
                .orderBy("city_norm", "month")

            print("Tính toán giá trung bình/m2 theo tháng hoàn tất.")
            print("Kết quả tổng hợp (Giá TB/m2 theo Tháng và Thành phố):")
            result_df.show(50, truncate=False)  # Hiển thị nhiều dòng hơn
            ...
            if not result_df.rdd.isEmpty():
                ...
                hanoi_avg_price_month_df = result_df.filter(col("city_norm") == "HaNoi")
                if not hanoi_avg_price_month_df.rdd.isEmpty():
                    print(
                        f"Đang ghi dữ liệu giá TB/m2 theo tháng (Hà Nội) vào Elasticsearch index '{ES_INDEX_AVG_PRICE_MONTH_HANOI}'...")
                    hanoi_avg_price_month_df.write \
                        .format("org.elasticsearch.spark.sql") \
                        .option("es.resource", ES_INDEX_AVG_PRICE_MONTH_HANOI) \
                        .mode("overwrite") \
                        .save()
                    print(
                        f"Ghi dữ liệu giá TB/m2 theo tháng (Hà Nội) vào Elasticsearch index '{ES_INDEX_AVG_PRICE_MONTH_HANOI}' thành công!")
                else:
                    print("Không có dữ liệu giá TB/m2 theo tháng cho Hà Nội để ghi vào Elasticsearch.")

                # --- Lọc và Ghi dữ liệu cho TP. Hồ Chí Minh vào Elasticsearch ---
                hochiminh_avg_price_month_df = result_df.filter(col("city_norm") == "HoChiMinh")
                if not hochiminh_avg_price_month_df.rdd.isEmpty():
                    print(
                        f"Đang ghi dữ liệu giá TB/m2 theo tháng (TP. Hồ Chí Minh) vào Elasticsearch index '{ES_INDEX_AVG_PRICE_MONTH_HOCHIMINH}'...")
                    hochiminh_avg_price_month_df.write \
                        .format("org.elasticsearch.spark.sql") \
                        .option("es.resource", ES_INDEX_AVG_PRICE_MONTH_HOCHIMINH) \
                        .mode("overwrite") \
                        .save()
                    print(
                        f"Ghi dữ liệu giá TB/m2 theo tháng (TP. Hồ Chí Minh) vào Elasticsearch index '{ES_INDEX_AVG_PRICE_MONTH_HOCHIMINH}' thành công!")
                else:
                    print("Không có dữ liệu giá TB/m2 theo tháng cho TP. Hồ Chí Minh để ghi vào Elasticsearch.")
                ...
                print("Không có dữ liệu tổng hợp giá TB/m2 theo tháng để ghi.")
