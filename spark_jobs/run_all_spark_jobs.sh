...
echo "Đang chạy job: count_by_district.py"
$SPARK_SUBMIT_CMD \
  --master $SPARK_MASTER_URL \
  $JOB_PATH_PREFIX/count_by_district.py
echo "Hoàn thành job: count_by_district.py"
echo "----------------------------------------"

echo "Đang chạy job: upload_json_to_es.py"
$SPARK_SUBMIT_CMD \
  --master $SPARK_MASTER_URL \
   $JOB_PATH_PREFIX/upload_json_to_es.py
 echo "Hoàn thành job: upload_json_to_es.py"
 echo "----------------------------------------"
